def print_all_values(students: dict) -> None:
    ...
    sorted_students = sorted(students.items(), key=lambda x: x[1]['height'], reverse=True)
    for surname, data in sorted_students:
        print(f"{surname}: {data['height']} cm" + (f", age {data['age']}" if data.get('age') else ""))
...
def print_sorted_by_keys(students: dict) -> None:
    ...
    for key in sorted(students.keys()):
        data = students[key]
        print(f"{key}: {data['height']} cm" + (f", age {data['age']}" if data.get('age') else ""))
...
def find_students_shorter_than(students: dict, new_height: float) -> list[str]:
    ...
    """
    shorter_students = [
        surname for surname, data in students.items()
...
def find_closest_height(students: dict, new_height: float) -> str:
    ...
    return min(students.items(), key=lambda x: abs(x[1]['height'] - new_height))[0] if students else ""
...
def process_new_student(students: dict) -> None:
    ...
    try:
        ...
        print(f"c) Closest height: {closest_student} ({abs(students[closest_student]['height'] - new_height):.2f} cm diff)")
        
        if input(f"\nAdd '{new_surname}'? (y/n): ").strip().lower() == 'y':
            age = input("Enter age (optional): ").strip()
            students[new_surname] = {'height': new_height, 'age': int(age) if age else None}
            print("Added.")
    except ValueError:
        print("Error: Invalid input.")
