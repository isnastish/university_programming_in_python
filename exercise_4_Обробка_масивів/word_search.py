...
def test_search_word():
    ...
    word1 = "banana"
    assert word1 in test_list1, (
        f"Test 1 failed: '{word1}' should be found in {test_list1}"
    )
    print(f"✓ Test 1 passed: '{word1}' found in {test_list1}")

    test_list2 = ["apple", "banana", "cherry"]
    word2 = "orange"
    assert word2 not in test_list2, (
        f"Test 2 failed: '{word2}' should not be found in {test_list2}"
    )
    print(f"✓ Test 2 passed: '{word2}' not found in {test_list2}")

    test_list3 = []
    word3 = "test"
    assert word3 not in test_list3, (
        f"Test 3 failed: '{word3}' should not be found in empty list"
    )
    print(f"✓ Test 3 passed: '{word3}' not found in empty list")

    test_list4 = ["first", "second", "third"]
    word4 = "first"
    assert test_list4.index(word4) == 0, (
        f"Test 4 failed: '{word4}' should be at index 0"
    )
    print(f"✓ Test 4 passed: '{word4}' found at index 0")

    test_list5 = ["Apple", "Banana"]
    word5 = "apple"
    assert word5 not in test_list5, (
        f"Test 5 failed: Case sensitive search should not find '{word5}'"
    )
